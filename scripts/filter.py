...
def prepare(source_file, target_file, max_in_out_size, lower=False):
    """
    ...
    df["Too-Long"] = ((df['Source'].str.count(' ')+1) > (df['Target'].str.count(' ')+1) * 2) |  \
                     ...
                     ((df['Source'].str.count(' ')+1) > max_in_out_size) |  \
                     ((df['Target'].str.count(' ')+1) > max_in_out_size)
...
prepare(source_file, target_file, None, lower=False)
